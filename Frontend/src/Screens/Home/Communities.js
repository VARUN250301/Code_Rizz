...
function Communities({ data, setJoinedCommunities }) {
  ...
  const handleJoinCommunity = async (
    ...
    const result = await setDoc(
      doc(db, `users/${user}/joinedCommunity`, communityName),
      {
        communityData,
      }
    );
  ...
  const images = [
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRV1INIPZHKfiZ8I5PWPjzC0oXaV6oyOJ0GcQ&usqp=CAU",
  ...
  return (
    <div>
      ...
      >
        {data.map((agency, index) => (
          <Card key={agency.name} className="communityCard">
            <Card.Img variant="top" src={images[index % 5]} />
            <Card.Body>
      ...
      <div>
        <img
          className="chatbot"
          src="https://is4-ssl.mzstatic.com/image/thumb/Purple126/v4/38/8c/db/388cdbbd-1749-50cd-7438-bee830f4e254/AppIcon-1x_U007emarketing-0-7-0-85-220.png/512x512bb.jpg"
        />
      </div>
    </div>
  );
